...
exports.registerUser = async (req, res, next) => {
	...
	try {
		...
		if (result.rowCount > 0) {
			errorHandler.throwBadRequestError("User with this email already exists.");
		}
		...
		if (servResponse.rowCount > 0) {
			...
			errorHandler.throwCreationFailureError("Failed creating a user. No changes were made.");
		}
	} catch (err) {
...
exports.loginUser = async (req, res, next) => {
	...
	try {
		...
		let servResponse = await pool.query(sql, values);
		if (servResponse.rowCount <= 0) {
			errorHandler.throwAuthorizationFailureError(`User with email [${body.email}] does not exist.`);
		}
